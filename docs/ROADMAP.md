...
- Step 2 일부는 반영됨
  - 사용자 suspension 상태 + admin suspension 조회/설정/해제 API
  - Post 상태 모델(`draft`, `published`, `deleted`) + draft 생성/발행 API
  - Comment 상태 모델(`active`, `deleted`) + 1-depth 대댓글 API
  - Reaction 중복 방지/변경 정책
  - 사용자 soft delete + 익명화
  - 사용자 uuid 공개 식별자 도입
  - Attachment 업로드/조회/preview/delete API
  - Attachment 본문 참조(`attachment://{id}`), orphan/pending-delete 정책
  - `FileStorage` 포트 + local filesystem / object storage 어댑터
  - attachment 이미지 최적화(`jpeg/png`) 및 업로드 검증
  - in-process job runner + orphan/pending-delete attachment cleanup 작업 등록
- Step 6 일부는 반영됨
  - 서비스 에러 래핑
  - HTTP 에러 정규화
  - `slog` 기반 구조화 에러 로그
- Step 2에서 아직 남은 대표 범위
  - `Tag`
  - `Report`
  - 이메일 인증 / 비밀번호 재설정
  - slug / 추가 운영 상태 모델 고도화
  - `Notification`, `PointHistory`(이벤트 경계 정리 이후)
