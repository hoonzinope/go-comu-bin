...
- UI/UX 개선 백로그 (Neo-Brutalism 유지 + 사용성 강화)
  ...
  - 시각 정체성/인터랙션
    ...
    - 클릭 물리감(pressed state): 주요 인터랙티브 컴포넌트 `:active`는 `--shadow-offset` 토큰(예: `4px`)을 기준으로 `transform: translate(var(--shadow-offset), var(--shadow-offset)); box-shadow: none;`를 적용해 shadow 두께 변경과 함께 유지되도록 한다
