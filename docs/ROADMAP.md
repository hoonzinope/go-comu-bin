...
- Guest lifecycle UI 흐름
  - 브라우저 최초 방문 시 `POST /api/v1/auth/guest` 자동 호출로 guest token을 발급하고 cookie/local storage에 저장한다
...
- Step 8 미착수 (Web UI / Admin Console)
  - HTML-first UI shell, cookie auth transport, SSR fallback metadata, current user summary, draft recovery/resume, admin console 계약 추가
  - go:embed 단일 바이너리 정적 자산/템플릿 서빙 (Step 7과 연계)
  - Cookie/Bearer 병행 미들웨어, CSRF double-submit cookie 방어
  - login/logout cookie 계약, `my_reaction_type` 조회 계약, email verification/password reset 진입 라우트 계약
  - 클라이언트 마크다운 XSS sanitization (DOMPurify + allowlist), guest lifecycle UI 흐름
