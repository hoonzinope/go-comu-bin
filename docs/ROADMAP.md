...
- UI/UX 개선 백로그 (Neo-Brutalism 유지 + 사용성 강화)
  ...
  - 모바일 반응형 UX
    ...
    - 관리자 테이블 모바일 카드화: .table-head/.table-row를 flex-direction: column 카드 레이아웃으로 전환하고, 각 셀 요소(div)에 data-label 속성을 부여해 column 의미를 보존한다.
    - 터치 타겟 개선: .bottom-nav-item 및 피드 내 투표 버튼(▲, ▼)의 터치 영역(padding/크기)을 확대하고, .is-active 상태의 시각적 식별성을 강화한다
  - 구현/검증 순서(UX 작업 공통)
