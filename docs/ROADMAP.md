...
- 배포 전제
  - same-origin auth transport 방식 결정
  - 정적 자산/템플릿 서빙 전략 결정
  - API 계약은 기존 JSON API를 재사용하되, UI 조립에 필요한 조회 계약을 보강
- UI 페이지
  - Public shell: feed, board, tag, search
  - Post detail: 본문, 댓글, 대댓글, 리액션, 첨부, 신고 진입
  - Post editor: draft 생성, 임시저장, 첨부, 발행, draft resume
  - Auth/account: signup, login, guest, guest upgrade, email verification, password reset
...
- 계약 보강
  ...
  - draft recovery/resume API 추가
    - draft list: 현재 사용자의 board별 draft 목록
    - draft detail: editor resume용 draft 단건 조회
    - UI에서 새 페이지/수정/재방문을 지원하려면 publish-only detail과 분리된 draft 계약이 필요
  - draft list/detail 계약은 기존 공개 post detail과 달리 owner/admin 전용으로 둔다.
  - 필요 시 `GET /api/v1/posts/{postUUID}/draft` 또는 동등한 detail route를 정의한다.
...
- Step 8 미착수 (Web UI / Admin Console)
  - HTML-first UI shell, current user summary, draft recovery/resume, admin console 계약 추가
