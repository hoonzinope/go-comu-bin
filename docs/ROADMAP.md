...
- Step 2 진행 중 (도메인 확장)
  ...
  - 미반영/다음 범위
    - password reset v2: SMTP 링크 템플릿, request rate limit, cleanup/audit 강화
    - Ranking(Hot/Best) 알고리즘 도입
      - Reddit 스타일 시간 감쇠(Time-decay) Hot 정렬 점수 정책 수립
      - Best/Top 정렬과의 점수 기준 분리(기간/누적 기준 명확화)
      - 점수 계산 트리거(조회수/리액션/댓글 등) 도메인 이벤트 정의
      - Outbox relay 기반 비동기 점수 갱신 파이프라인으로 읽기 경로 부하 분리
- Step 3 완료 (보안/어뷰징 1차)
  - 완료/반영
