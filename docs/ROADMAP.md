...
- Outbox 내구화: SQLite Outbox 테이블 + relay(또는 CDC) 전환
...
- Step 2 진행 중 (도메인 확장)
  - 완료/반영
    ...
    - Tag 연계: post create/update + `GET /posts/by-tag/:tagName` 조회
  ...
  - 미반영/다음 범위
    - 이메일 인증 / 비밀번호 재설정
    - 공개 UUID 전환 이후 문서/운영 규칙 잔여 정리
    - `Notification`, `PointHistory` (durable outbox 단계 이후)
- Step 3 완료 (보안/어뷰징 1차)
  - 완료/반영
    - `RateLimiter` 포트 + in-memory fixed-window 구현
    - `/api/v1` 쓰기 요청(`POST/PUT/DELETE/PATCH`) IP 기준 `429` 제한
    - `InputSanitizer` 포트 + escape 구현
    - 게시판/게시글/댓글/신고 관련 입력의 HTML escape 파이프라인
- Step 4 진행 중 (EDA)
  - 완료/반영
  ...
  - 미반영/다음 범위
    - durable outbox adapter(SQLite table) 전환
    - Mention 이벤트 + Notification 연결
- Step 5 미착수 (외부 인프라 전환)
  - SQLite repository/FTS5
  - durable outbox relay/CDC
  - MQ bridge(선택)
- Step 6 진행 중 (관측성/예외)
