...
- password reset request: `AccountUseCase`가 email 기준으로 활성 일반 사용자를 찾고, 이전 미사용 reset token을 무효화한 뒤 새 1회용 token을 저장하고 mail sender 포트로 전달한다. HTTP는 이 엔드포인트에 `client_ip + normalized_email` 기준 전용 rate limit을 추가 적용한다.
