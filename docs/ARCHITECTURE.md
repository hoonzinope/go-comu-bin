...
- 서비스는 원인 분류를 잃지 않도록 에러를 래핑한다.
  - 저장소 실패: `customerror.ErrRepositoryFailure`
    - SQLite 세부 원인: `customerror.ErrSQLiteBusy`, `customerror.ErrSQLiteConstraint`, `customerror.ErrSQLiteForeignKey`
  - 캐시 실패: `customerror.ErrCacheFailure`
  - 토큰 발급 실패: `customerror.ErrTokenFailure`
  - 외부 연동 실패: `customerror.ErrMailDeliveryFailure`, `customerror.ErrStorageFailure`
...
- runtime 구현은 `Ristretto` 기반 어댑터를 사용한다.
  - write buffering은 `Wait()`로 flush해 immediate read-after-write 의미를 유지한다.
  - prefix 무효화는 별도 prefix index로 처리한다.
- test double/reference 구현은 `internal/infrastructure/cache/inmemory`를 유지한다.
  - 만료(`TTL`) 지원
...
- `internal/infrastructure/cache/ristretto`
  - runtime용 cache adapter
- `internal/infrastructure/cache/inmemory`
  - test double/reference cache adapter
...
- 역할
  ...
  - reverse proxy 뒤 배포 시 `delivery.http.trustedProxies`를 주입하고, TLS 종료는 외부 프록시 책임으로 둔다.
...
- `delivery.NewHTTPServer` 는 trusted proxy 목록을 받아 `gin`의 client IP 계산 기준을 분리한다.
