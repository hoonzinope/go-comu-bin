...
- guest issue: `SessionUseCase`가 guest user row 생성 후 동일한 토큰 발급/세션 저장 경로를 재사용한다.
...
- guest upgrade: 현재 guest user row를 정식 user로 승격하고, 기존 `id`, `uuid`, 작성물 소유권, 활성 세션은 그대로 유지한다.
...
- guest 탈퇴는 사용자에게 비밀번호를 노출하지 않는 정책이므로 현재 token 소유자 기준으로 비밀번호 검증을 생략한다.
