...
- guest issue: `SessionUseCase`가 `pending guest 생성 -> token 발급 -> session 저장 -> active 전환` 흐름으로 처리한다.
...
- guest upgrade: `AccountUseCase`가 현재 guest user row를 정식 user로 승격하고, 기존 `id`, `uuid`, 작성물 소유권은 유지하되, 현재 guest token 세션은 폐기 후 새 bearer token으로 교체한다.
- guest upgrade는 user row 승격과 token 교체를 하나의 성공 경계로 처리하며, 중간 실패 시 기존 guest/session 상태로 복구를 시도한다.
- email verification request: `AccountUseCase`가 현재 로그인 사용자를 기준으로 이전 미사용 verification token을 무효화하고 새 1회용 token을 저장한 뒤 메일을 발송한다.
- email verification confirm: `AccountUseCase`가 token 검증 후 `EmailVerifiedAt`을 기록하고, 같은 사용자의 verification token을 모두 무효화한다.
- password reset request: `AccountUseCase`가 email 기준으로 활성 일반 사용자를 찾고, 이전 미사용 reset token을 무효화한 뒤 새 1회용 token을 저장하고 mail sender 포트로 전달한다.
- password reset confirm: `AccountUseCase`가 token 검증 후 비밀번호 변경 + token 소모 + 전체 세션 무효화를 하나의 성공 경계로 처리하고, 세션 무효화 실패 시 사용자 비밀번호와 token 상태를 복구한다.
...
- signup의 email verification 자동 발송은 `UserService`가 verification token 저장 + 메일 발송까지 함께 오케스트레이션한다.
- guest 계정도 내부 랜덤 비밀번호를 해시해서 저장하지만, 일반 username/password login 대상으로 사용하지 않는다.
- 인증/탈퇴 시에는 저장된 해시와 입력 비밀번호를 비교한다.
- `AuthorizationPolicy.CanWrite`는 suspension 외에도 email verification 상태를 함께 확인한다.
- email verification token도 password reset과 같은 해시 저장 + 기본 TTL 30분 + 최신 요청만 유효 정책을 사용한다.
- password reset token은 평문이 아니라 해시 저장을 기본으로 하며, 기본 TTL은 30분이고 최신 요청만 유효하다.
...
- `internal/application/port/notification_usecase.go`
  - 사용자 notification 목록/미읽음 수/읽음 처리 포트
