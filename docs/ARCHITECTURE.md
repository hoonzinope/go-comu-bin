...
- email verification request: `AccountUseCase`가 현재 로그인 사용자를 기준으로 이전 미사용 verification token을 무효화하고 새 1회용 token을 저장한 뒤 메일을 발송한다. HTTP는 이 엔드포인트에 `user_id` 기준 전용 rate limit을 추가 적용한다.
