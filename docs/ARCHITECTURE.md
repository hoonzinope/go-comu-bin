...
- Event Type (최소 세트)
  ...
  - `AttachmentChanged`
- 디스패치
  - 서비스는 트랜잭션 내부에서 `tx.Outbox().Append(...)`로 이벤트를 outbox에 기록한다.
  ...
  - `maxAttempts` 초과 메시지는 `dead` 상태로 보존한다(자동 삭제하지 않음).
  - 핸들러 panic/error는 recover 후 retry/dead 전이로 기록한다.
  - 서버 종료는 graceful shutdown(`http shutdown -> relay cancel -> relay wait`) 경계를 따른다.
