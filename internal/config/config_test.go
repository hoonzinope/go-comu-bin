...
func TestLoadFromViper_ValidConfig(t *testing.T) {
	...
	v.Set("event.outbox.processingLeaseMillis", 1000)
	v.Set("event.outbox.leaseRefreshMillis", 250)
	...
	v.Set("cache.maxCost", int64(100000))
	...
	assert.Equal(t, 1000, cfg.Event.Outbox.ProcessingLeaseMillis)
	assert.Equal(t, 250, cfg.Event.Outbox.LeaseRefreshMillis)
	assert.Equal(t, 30, cfg.Cache.ListTTLSeconds)
	assert.Equal(t, 60, cfg.Cache.DetailTTLSeconds)
	assert.Equal(t, int64(100000), cfg.Cache.MaxCost)
...
func TestLoadFromViper_InvalidRateLimitReadRequests(t *testing.T) {
	v := viper.New()
	v.Set("delivery.http.port", 18577)
	v.Set("delivery.http.auth.secret", "test-secret-1234567890-abcdef-1234")
	v.Set("delivery.http.rateLimit.readRequests", 0)
	v.Set("cache.listTTLSeconds", 30)
	v.Set("cache.detailTTLSeconds", 30)
	v.Set("storage.provider", "local")
	v.Set("storage.local.rootDir", "./data/uploads")
...
func TestLoadFromViper_InvalidCacheCapacity(t *testing.T) {
	t.Run("max_cost_is_zero", func(t *testing.T) {
		v := viper.New()
		v.Set("delivery.http.port", 18577)
		v.Set("delivery.http.auth.secret", "test-secret-1234567890-abcdef-1234")
		v.Set("cache.listTTLSeconds", 30)
		v.Set("cache.detailTTLSeconds", 30)
		v.Set("cache.maxCost", int64(0))
		v.Set("cache.numCounters", int64(1000000))
		v.Set("cache.bufferItems", int64(64))
		v.Set("cache.metrics", false)
		v.Set("storage.provider", "local")
	...
	t.Run("buffer_items_is_zero", func(t *testing.T) {
		v := viper.New()
		v.Set("delivery.http.port", 18577)
		v.Set("delivery.http.auth.secret", "test-secret-1234567890-abcdef-1234")
		v.Set("cache.listTTLSeconds", 30)
		...
		v.Set("cache.maxCost", int64(100000))
		v.Set("cache.numCounters", int64(1000000))
		v.Set("cache.bufferItems", int64(0))
		v.Set("cache.metrics", false)
		v.Set("storage.provider", "local")
		v.Set("storage.local.rootDir", "./data/uploads")
		v.Set("storage.attachment.maxUploadSizeBytes", int64(10<<20))
...
func TestLoadFromViper_RequiresSMTPHostWhenMailEnabled(t *testing.T) {
	...
	v.Set("delivery.mail.emailVerification.baseURL", "https://app.example.com/verify-email")
	v.Set("delivery.mail.passwordReset.baseURL", "https://app.example.com/reset-password")
...
func TestLoadFromViper_RequiresPasswordResetBaseURLWhenMailEnabled(t *testing.T) {
	v := viper.New()
	v.Set("delivery.http.port", 18577)
	v.Set("delivery.http.auth.secret", "test-secret-1234567890-abcdef-1234")
	v.Set("delivery.mail.enabled", true)
	...
	require.Error(t, err)
	assert.Nil(t, cfg)
	assert.Contains(t, err.Error(), "delivery.mail.passwordReset.baseURL")
}

func TestLoadFromViper_RequiresEmailVerificationBaseURLWhenMailEnabled(t *testing.T) {
	v := viper.New()
	v.Set("delivery.http.port", 18577)
	v.Set("delivery.http.auth.secret", "test-secret-1234567890-abcdef-1234")
	v.Set("delivery.mail.enabled", true)
	...
	require.Error(t, err)
	assert.Nil(t, cfg)
	assert.Contains(t, err.Error(), "delivery.mail.emailVerification.baseURL")
}

func TestLoadFromViper_AcceptsPasswordResetRateLimitAndCleanupConfig(t *testing.T) {
	v := viper.New()
	v.Set("delivery.http.port", 18577)
	v.Set("delivery.http.auth.secret", "test-secret-1234567890-abcdef-1234")
	v.Set("delivery.mail.enabled", true)
	...
	v.Set("delivery.mail.passwordReset.baseURL", "https://app.example.com/reset-password")
	v.Set("delivery.mail.emailVerification.baseURL", "https://app.example.com/verify-email")
	v.Set("delivery.http.auth.passwordResetRequestRateLimit.enabled", true)
	v.Set("delivery.http.auth.passwordResetRequestRateLimit.windowSeconds", 60)
	v.Set("delivery.http.auth.passwordResetRequestRateLimit.maxRequests", 5)
	v.Set("delivery.http.auth.emailVerificationRequestRateLimit.enabled", true)
	v.Set("delivery.http.auth.emailVerificationRequestRateLimit.windowSeconds", 60)
	v.Set("delivery.http.auth.emailVerificationRequestRateLimit.maxRequests", 5)
	v.Set("cache.listTTLSeconds", 30)
	v.Set("cache.detailTTLSeconds", 30)
	v.Set("storage.provider", "local")
	v.Set("storage.local.rootDir", "./data/uploads")
	v.Set("jobs.enabled", true)
	v.Set("jobs.emailVerificationCleanup.enabled", true)
	v.Set("jobs.emailVerificationCleanup.intervalSeconds", 300)
	v.Set("jobs.emailVerificationCleanup.gracePeriodSeconds", 600)
	v.Set("jobs.emailVerificationCleanup.batchSize", 100)
	v.Set("jobs.passwordResetCleanup.enabled", true)
	v.Set("jobs.passwordResetCleanup.intervalSeconds", 300)
	v.Set("jobs.passwordResetCleanup.gracePeriodSeconds", 600)
	v.Set("jobs.passwordResetCleanup.batchSize", 100)

	cfg, err := loadFromViper(v)
	require.NoError(t, err)
	require.NotNil(t, cfg)
	assert.True(t, cfg.Delivery.HTTP.Auth.EmailVerificationRequestRateLimit.Enabled)
	assert.Equal(t, 60, cfg.Delivery.HTTP.Auth.EmailVerificationRequestRateLimit.WindowSeconds)
	assert.Equal(t, 5, cfg.Delivery.HTTP.Auth.EmailVerificationRequestRateLimit.MaxRequests)
	assert.True(t, cfg.Delivery.HTTP.Auth.PasswordResetRequestRateLimit.Enabled)
	assert.Equal(t, 60, cfg.Delivery.HTTP.Auth.PasswordResetRequestRateLimit.WindowSeconds)
	assert.Equal(t, 5, cfg.Delivery.HTTP.Auth.PasswordResetRequestRateLimit.MaxRequests)
	assert.True(t, cfg.Jobs.EmailVerificationCleanup.Enabled)
	assert.Equal(t, 300, cfg.Jobs.EmailVerificationCleanup.IntervalSeconds)
	assert.Equal(t, 600, cfg.Jobs.EmailVerificationCleanup.GracePeriodSeconds)
	assert.Equal(t, 100, cfg.Jobs.EmailVerificationCleanup.BatchSize)
	assert.True(t, cfg.Jobs.PasswordResetCleanup.Enabled)
	assert.Equal(t, 300, cfg.Jobs.PasswordResetCleanup.IntervalSeconds)
